...
反过来说，cc-fleet 用**广度换了深度**，短板也很实在：**聊天平台只有企微 + 个人微信**（cc-connect 覆盖 13 家、cc-im 覆盖 3 家）；**agent 跑 Claude Code / Codex / opencode**（架构可插拔；cc-connect 走 ACP 支持 10+）；也没有语音 / 多媒体 / cron / 多 provider 切换这些"桥"类富功能。**如果你要的是广覆盖的远程遥控，cc-connect 等是更合适的选择；如果你要的是"发一句需求、收一份能评审的 MR"，那才是 cc-fleet。**
...
- **依赖**：Python ≥ 3.10、聊天平台凭据（企业微信机器人 或 个人微信 ilink `bot_token`）、本机 `claude` CLI（Claude Code）在 PATH 中；仓库配 `agent: codex` 时需 `codex` CLI 且已 `codex login`；配 `agent: opencode` 时需 `opencode` CLI 且已配 provider 凭据
...
- **可插拔 Agent Runner**：架构把「驱动 coding agent 子进程」抽象成可插拔 runner，编排层工具无关；当前实现 **Claude Code** / **Codex CLI** / **opencode**（按 repo 用 `agent:` 选用；护栏强弱 claude > codex > opencode，见下） —— 设计与扩展思路见 [docs/architecture.md](./docs/architecture.md)
...
- **生命周期清理**：启动时及每小时清理超过 `worktree_retention_hours` 的 completed/cancelled 本地 worktree；可恢复的 failed/timeout 与 remote worktree 不自动删除
...
- **私聊免引用续聊**：个人微信 1:1 / 企微单聊里（`default_mode: chat`），默认 30 分钟内直接发消息会自动接着**最近一个活跃对话**、无需引用；隔太久、或想换个话题就用 `/chat <消息>` 开新。窗口由 `chat.auto_continue_window_sec` 调整（设 `0` 关闭，回到"必须引用才续聊"）。仅私聊生效，群聊仍需引用或 `@<repo>`。
...
每条机器人消息末尾都会带 `[session: <slug>]` 行 —— 这是反向标识符，用户**引用包含这一行的消息**即可让回复落到同一个 session。（例外：私聊在 `chat.auto_continue_window_sec` 窗口内免引用也会自动续到最近一个活跃对话，见上文「私聊免引用续聊」。）
...
护栏按工具分两套机制（按「每工具尽力而为 + 显式标注差距」原则）：

| 工具 | 机制 | 覆盖 | 已知缺口 |
|---|---|---|---|
| Claude Code | `PreToolUse` hook（命令级检查） | force push / 越界写 / 敏感路径访问 | shell 字符串混淆可绕过 |
| Codex CLI | OS sandbox（`--sandbox`，macOS Seatbelt / Linux Landlock） | 越界**写**被内核拦截（强于 hook） | **force-push 等网络操作不拦**、敏感目录**读**不拦（靠 prompt 纪律自律）；其它平台退化为无隔离 |
| opencode | **纯 prompt 软防护**（无插件 / 无沙箱） | 只读阶段用内置 `plan` agent 限权 | 写阶段**零机械护栏**（越界写 / force-push / 敏感读全靠 prompt 纪律），三工具最弱，启动 WARN |

claude 侧源码 `src/cc_fleet/security/hooks/pretool_guard.py`，单测 `tests/test_pretool_guard.py`；codex 侧见 `src/cc_fleet/core/runners/codex.py`，opencode 侧见 `src/cc_fleet/core/runners/opencode.py`。完整边界、注入语义、对外部署 checklist 见 [docs/security.md](./docs/security.md) 与 [SECURITY.md](./SECURITY.md)。
