...
def _make_docker_repo_cfg(tmp_path: Path, *, mapping: bool) -> RepoConfig:
    """docker 模式 repo：代码在主机本地（同 local），仅编译/运行进容器。"""
    repo = tmp_path / "proj"
    repo.mkdir(exist_ok=True)
    (repo / ".git").mkdir(exist_ok=True)
    extra = (
        dict(docker_host_root=str(tmp_path), docker_container_root="/workspace")
        if mapping
        else {}
    )
    return RepoConfig(
        ...
        default_branch="main",
        mode="docker",
        docker_container="mydev",
        **extra,
    )


async def test_docker_happy_path_completed(db, cfg, tmp_path, reply, replies):
    """docker 复用 local 全流水线：主机建 worktree + commit + 主控提 MR；
    dev 阶段渲染 docker 协议，把容器名与容器内 worktree 路径注入 prompt。"""
    repo_cfg = _make_docker_repo_cfg(tmp_path, mapping=True)
    claude_stub = make_claude_stub([
        "plan ready\n\nSLUG: add-docker-build\nSTATUS: READY\n",
        "完成报告：在主机 worktree 改了代码、在容器内编译通过、已 commit。\n\n"
        "MR_TITLE: feat: 容器内构建打通\n\n"
        "MR_DESCRIPTION_BEGIN\n## 背景\n打通容器构建\nMR_DESCRIPTION_END\n",
    ])
    s = Session(db=db, config=cfg, repo_cfg=repo_cfg, reply=reply, claude_run=claude_stub)
    await s.start(initial_request="容器内编译", chatid="c1", userid="u1")

    row = await db.get_session(s.slug)
    assert row["state"] == SessionState.COMPLETED.value
    assert row["display_slug"] == "add-docker-build"
    assert row["mr_url"].endswith("/42")
    # docker 走 local 路径：分支在主机本地创建（非 remote 的 None）
    assert row["branch"] == f"claude/{s.slug}"
    # worktree 在主机本地 <path>-worktrees/<slug>
    assert Path(row["worktree_path"]) == tmp_path / "proj-worktrees" / s.slug

    # dev 阶段落盘的 system prompt 应是 docker 协议，含容器名与容器内 worktree 路径
    dev_prompt = (
        cfg.workspace_root / "sessions" / s.slug / "dev_system_prompt.md"
    ).read_text(encoding="utf-8")
    assert "docker exec mydev" in dev_prompt
    assert f"/workspace/proj-worktrees/{s.slug}" in dev_prompt


async def test_docker_container_worktree_path_and_env(db, cfg, tmp_path, reply):
    """_container_worktree_path 前缀换算 + _claude_extra_env 注入语义。"""
    claude_stub = make_claude_stub(["unused"])

    # 1) 无映射：容器内路径同主机，不注入 EXTRA_WORKTREE_ROOTS
    rc_same = _make_docker_repo_cfg(tmp_path, mapping=False)
    s_same = Session(db=db, config=cfg, repo_cfg=rc_same, reply=reply, claude_run=claude_stub)
    wt = tmp_path / "proj-worktrees" / "slug-x"
    assert s_same._container_worktree_path(wt) == str(wt)
    assert "CC_FLEET_EXTRA_WORKTREE_ROOTS" not in s_same._claude_extra_env(wt)

    # 2) 有映射：前缀替换为容器内路径，并注入白名单兜底
    rc_map = _make_docker_repo_cfg(tmp_path, mapping=True)
    s_map = Session(db=db, config=cfg, repo_cfg=rc_map, reply=reply, claude_run=claude_stub)
    host_wt = tmp_path / "proj-worktrees" / "slug-x"
    assert s_map._container_worktree_path(host_wt) == "/workspace/proj-worktrees/slug-x"
    env = s_map._claude_extra_env(host_wt)
    assert env["CC_FLEET_EXTRA_WORKTREE_ROOTS"] == "/workspace/proj-worktrees/slug-x"


async def test_docker_start_failure_aborts_dev_and_stays_failed(
    db, cfg, tmp_path, reply, replies, monkeypatch: pytest.MonkeyPatch
):
    """docker 容器启动失败：必须中止 dev 并停在 FAILED，绝不能覆盖回工作态去推 MR。

    回归 A-1：旧实现里 _ensure_docker_container 失败只 _fail 不中止，dev 跑完后
    _set_state(MR_SUBMITTING) 把 FAILED 覆盖回工作态，导致在用户已收到失败通知后
    仍推出一个未成功编译/测试的 MR。
    """
    repo_cfg = _make_docker_repo_cfg(tmp_path, mapping=True).model_copy(
        update={
            "docker_start_command": "docker start mydev",
            "docker_stop_command": "docker rm -f mydev",
        }
    )

    # 所有 docker inspect / 启动 / 销毁命令都返回失败 → 模拟容器起不来
    class _FailProc:
        returncode = 1

        async def communicate(self):
            return b"", b"boom: image not found"

    async def fake_shell(_cmd, **_kwargs):
        return _FailProc()

    monkeypatch.setattr(asyncio, "create_subprocess_shell", fake_shell)

    # claude：plan READY；dev 若被（错误地）执行会返回成功，从而暴露回归
    modes: list[str] = []

    async def claude_stub(**kwargs) -> ClaudeRunResult:
        mode = perm_mode(kwargs)
        modes.append(mode)
        text = (
            "plan ok\n\nSLUG: docker-broken\nSTATUS: READY\n"
    ...
    async def spy_mr(**kwargs) -> str:
        mr_calls.append(kwargs)
        return "https://gitlab/example/-/merge_requests/99"

    monkeypatch.setattr(mr_module, "create_mr_via_push", spy_mr)

    s = Session(db=db, config=cfg, repo_cfg=repo_cfg, reply=reply, claude_run=claude_stub)
    await s.start(initial_request="容器内构建", chatid="c1", userid="u1")

    row = await db.get_session(s.slug)
    assert row["state"] == SessionState.FAILED.value  # 停在 FAILED，未被覆盖回工作态
    assert row["mr_url"] is None  # 未建 MR
    assert mr_calls == []  # create_mr_via_push 从未被调用
    assert "acceptEdits" not in modes  # dev 阶段 claude 从未运行
    assert any("失败" in t for _, t in replies)  # 收到失败通知
    assert not any("MR" in t for _, t in replies)  # 无任何 MR/完成误导消息
