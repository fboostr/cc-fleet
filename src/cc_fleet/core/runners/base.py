...
# 一次运行因何被引擎回收：空闲 / 工具执行静默 / 触达绝对上限。None = 未超时。
TimeoutKind = Literal["idle", "tool", "hard_cap"]


@dataclass(frozen=True)
class TimeoutPolicy:
    """一次 agent 子进程运行的超时策略（工具无关，取代原来的单一 ``timeout_sec``）。

    三档语义见 ``engine.run_subprocess`` 的监控循环——不再按「进程总时长」一刀切，
    而是按「是否有工具在飞」分档，从而不误杀长编译 / 测试：

    - ``idle_sec``：**无**工具在飞时的「回合间空闲」上限。agent 出了结果却迟迟不发起
      下一个动作、且没有工具在跑——这才是真正的卡死 / 等输入信号，收得紧。
    - ``tool_sec``：**有**工具在飞（``tool_use`` 已发、对应 ``tool_result`` 未回）时的
      静默上限。覆盖大型 C++ 编译 / 数十分钟测试等合法长工具，放得松。
    - ``hard_cap_sec``：从进程启动起算的绝对总时长兜底，防「一直吐事件但陷入死循环」
      的病态情形——正常任务永远碰不到。

    三者都以「杀进程组 + 落 ``TIMEOUT``（可唤醒续跑）」收尾，仅 ``timeout_kind`` 不同，
    供上层区分通知文案。
    """
...
# TimeoutKind → 面向用户的中文原因短语，供上层拼通知 / 落 last_error（如
# f"{阶段}{timeout_phrase(kind)}超时" → "dev 阶段工具执行超时"）。None / 未知回退泛化"运行"。
_TIMEOUT_KIND_PHRASE: dict[str, str] = {
    "idle": "空闲无响应",
    "tool": "工具执行",
    "hard_cap": "总时长",
}
...
def timeout_phrase(kind: TimeoutKind | None) -> str:
    """把 ``result.timeout_kind`` 翻成中文短语；None / 未知回退泛化"运行"。"""
    return _TIMEOUT_KIND_PHRASE.get(kind or "", "运行")
...
class ClaudeRunResult:
    ...
    # ``timed_out=True`` 时具体是哪一档超时（空闲 / 工具静默 / 绝对上限），供上层细化
    # 通知文案；未超时为 None。``killed`` 表示被 ``/kill`` 手动强杀（非超时）——上层据此
    # 走「取消」语义（落 CANCELLED）而非 TIMEOUT。
