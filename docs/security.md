...
git push --force
git push -f
git push -uf / -fu / -vf      # 合并短选项里含 f 即 force
...
补充两类扫描，堵住 sanitize 引入的盲区（否则「把写目标加引号」即可绕过越界写检查）：

- **展开后越界的 `~/` 写**：未加引号的 `~/x` 在 bash 里展开到 home（worktree 外），同样校验白名单
- **引号内的绝对路径写目标**：sanitize 会抹掉引号内容以放行 push option value 里的 `/healthcheck` 等字面量，但这也会让 `rm "/越界/路径"` 逃过扫描。故在**确有写命令**（`rm` / `cp` / `tee` …）或**重定向直接指向引号路径**（`> "/越界"`）时，对原始命令补扫引号绝对路径（`grep "/x" 2>/dev/null` 这类读参数不受影响）

### 5. Write / Edit 相对路径穿越

`file_path` 为相对路径时，按 cwd（= 主 worktree）解析后再校验：普通子路径放行，`../../x` 这类相对穿越逃出 worktree 会被拒绝。

## 不能拦什么

- **Prompt-injection**：恶意 commit message / 文件内容能让 claude 在自身的工具使用层"自愿"绕过约定。Write / Edit 的相对路径穿越已被 §5 覆盖，但 **Bash 里的相对重定向**（如 `echo x > ../../outside`）仍拦不到——启发式只校验绝对路径与 `~/`，相对写目标是残余盲区
...
- `tests/test_pretool_guard.py` —— 全面单测覆盖：force push 各形态（含 `-uf` 合并短选项）、敏感路径、heredoc / quote / fd-dup sanitize、引号绝对路径写目标、`~/` 展开越界、Write/Edit 相对穿越、白名单 / 越界
